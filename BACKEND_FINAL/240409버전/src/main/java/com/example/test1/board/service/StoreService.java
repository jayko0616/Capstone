...
public class StoreService {
    ...
    public Page<StoreDTO> paging(Pageable pageable) {
        ...
        Page<StoreEntity> storeEntities = storeRepository.findAll(PageRequest.of(page, pageLimit, Sort.by(Sort.Direction.DESC, "id")));

        // 목록: id, storeCode, storeName, boardHits, createdTime, rating
        Page<StoreDTO> storeDTOS = storeEntities.map(store -> new StoreDTO(store.getId(), store.getStoreCode(), store.getBoardTitle(), store.getBoardHits(), store.getAvgRating(),store.getCreatedTime()));
        return storeDTOS;
    }
