...
const lat = 37.5840409; // 여기에 위도 숫자를 넣어주세요
const lng = 127.0540733; // 여기에 경도 숫자를 넣어주세요
...
function Map() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:8082/api/store/');
                    ...
                    addArr.map(async (item, index) => {
                        try {
                            const response = await axios.put(
                                'http://localhost:8082/api/store/update',
                                {
                                    id: stores[index].id,
                                    coordinateX: "" + item.x,
                                    coordinateY: "" + item.y
                ...
                if (store.coordinateY && store.coordinateX) {
                    ...
                    const infowindow = new naver.maps.InfoWindow({
                        content: [
                            ...
                            `   <div style="font-weight: bold; font-size : 13px; color: grey">${store.storeContent}</div>`,
